...
class OverlayService : Service() {
    ...
    private var translationOverlayController: com.example.render.TranslationOverlayController? = null
    ...
    override fun onCreate() {
        ...
        floatingButtonController = FloatingButtonController(
            ...
            onShortClick = {
                ...
                if (state == com.example.capture.CaptureState.Prepared || state == com.example.capture.CaptureState.Success || state == com.example.capture.CaptureState.Failed) {
                    val ocrState = com.example.vision.VisionManager.ocrState.value
                    if (ocrState == com.example.vision.OcrState.Processing || ocrState == com.example.vision.OcrState.TextProcessing) {
                        ...
                        android.widget.Toast.makeText(this, "Processing requested...", android.widget.Toast.LENGTH_SHORT).show()
                    }
                } else {
        ...
        translationOverlayController = com.example.render.TranslationOverlayController(this)
        ...
        scope.launch {
            com.example.capture.ScreenCaptureManager.captureState.collect { state ->
                if (state == com.example.capture.CaptureState.Capturing) {
        ...
        scope.launch {
            com.example.translation.TranslationManager.translationState.collect { state ->
                when (state) {
                    com.example.translation.TranslationState.Success -> {
                        val result = com.example.translation.TranslationManager.lastTranslationResult.value
                        val captureResult = com.example.capture.ScreenCaptureManager.lastCaptureResult.value
                        if (result != null) {
                            ...
                            com.example.render.RenderManager.processTranslation(
                    ...
                    com.example.translation.TranslationState.ModelRequired -> {
                    ...
                    com.example.translation.TranslationState.Failed -> {
                    ...
                    com.example.translation.TranslationState.Skipped -> {
                        val reason = com.example.translation.TranslationManager.lastTranslationResult.value?.skippedReason
                        if (reason == com.example.translation.TranslationSkippedReason.NO_TEXT) {
                            android.widget.Toast.makeText(this@OverlayService, "No text found on this screen.", android.widget.Toast.LENGTH_SHORT).show()
                        } else if (reason == com.example.translation.TranslationSkippedReason.SAME_AS_TARGET) {
                            android.widget.Toast.makeText(this@OverlayService, "Source is same as target language.", android.widget.Toast.LENGTH_SHORT).show()
                        } else if (reason != com.example.translation.TranslationSkippedReason.MODEL_NOT_READY) {
        ...
        scope.launch {
            com.example.render.RenderManager.renderState.collect { state ->
                if (state == com.example.render.RenderState.Success) {
                    val result = com.example.render.RenderManager.lastRenderResult.value
                ...
                } else if (state == com.example.render.RenderState.Idle) {
